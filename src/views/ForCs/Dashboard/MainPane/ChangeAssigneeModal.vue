  ...
  <div class="inline-block shadow-xl transform align-middle mt-16" >
    <div class=" rounded bg-white p-10">
      ...
      <div class="grid grid-cols-2 gap-1 mb-2">
        <div
          v-for="(cs, i) in css.filter(c => c.id != task.assignee.id)"
          :key="i"
          @click.stop="nextAssigneeId = cs.id"
          class="rounded shadow p-2 w-72"
          :class="{ 'bg-blue-100': nextAssigneeId && nextAssigneeId == cs.id }"
        >
          <input v-model="nextAssigneeId" :value="cs.id" :id="`cs${i}`" type="radio" name="cs">
          <label class="ml-2" :for="`cs${i}`">{{ cs.full_name }}</label>
        </div>
      </div>
      <hr class="my-4">
      <div class="flex ">
